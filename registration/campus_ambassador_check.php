...
showerror();
    ...
    //$ex='GMR Institute of Technology';
    $ex=mysql_fetch_array(mysql_query("select Institutename from registration where Email='$requestedEmail'"));
    $m=mysql_num_rows(mysql_query("select * from campus_ambassador where Email='$requestedEmail'"));
    if($n==0 ){
        $valid=false;
    }
    if($ex[0]=='GMR Institute of Technology')
    {
        $valid=false;
    }
    if($m!=0)
